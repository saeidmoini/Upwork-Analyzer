...
var categoryParam = urlParams.get('category_uid');
var subParam = urlParams.get('subcategory_uid');
var categoryArray = {
...
var subCategoryArray = {
    ...
    "531770282597445646": "Professional & Business Writing" ,
    "1534904462131675136" : "Sales & Marketing Copywriting" 
 }
...
function runAG() {
    chrome.storage.local.get(["talentc"]).then((result) => {
        if(result.talentc){
            console.log("Numbers Exist On Storage");
            var numbers = JSON.parse(result.talentc);
        }else{
            var numbers = [];
        }
        var newNumber = { search: searchParam , category : category , subcategory : subCategory};

        var count = document.querySelectorAll('[class="mobile-pagination"]');
        if(count[0]){
            count = parseInt(count[0].innerText.split("of")[1])*10;
        }else{
            count=5;
        }
        
        let newCount = {talentcount : count};
        Object.assign(newNumber, newCount);
        numbers.push(newNumber);
        numbers = JSON.stringify(numbers);
        chrome.storage.local.set({ talentc: numbers }).then(() => {
            alert("Collected");
        }); 
    });
}
